...
const configId = '11zm30RwKbknBOKJ3uOAap-Zzbj4-dpzh';
const linksId = '1vFlYYnpVlhpL7aF8DronxfY6CGlAhdTf';
...
async function run() {
    try {
        // 1. Backups
        const configPath = path.join(assetsDir, 'clinics_config.json');
        const configBackupPath = path.join(assetsDir, 'clinics_config_backup.json');
        if (fs.existsSync(configPath)) {
            fs.copyFileSync(configPath, configBackupPath);
            console.log(`Backed up ${configPath} to ${configBackupPath}`);
        }

        const linksPath = path.join(assetsDir, 'clinics_links.json');
        const linksBackupPath = path.join(assetsDir, 'clinics_links_backup.json');
        if (fs.existsSync(linksPath)) {
            fs.copyFileSync(linksPath, linksBackupPath);
            console.log(`Backed up ${linksPath} to ${linksBackupPath}`);
        }

        const rootConfigPath = path.join(baseDir, 'clinics_config_utf8.json');
        const rootConfigBackupPath = path.join(baseDir, 'clinics_config_utf8_backup.json');
