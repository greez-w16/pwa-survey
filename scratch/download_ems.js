...
const configId = '1eL-WsAn_CryT8BIr-dWiuj2c0z5qCxMk';
const linksId = '1Ei5xRYHzasXvKc7no6od6ExvJF_l-_Yf';
...
async function run() {
    try {
        // 1. Backups
        const configPath = path.join(assetsDir, 'ems_config.json');
        const configBackupPath = path.join(assetsDir, 'ems_config_backup.json');
        if (fs.existsSync(configPath)) {
            fs.copyFileSync(configPath, configBackupPath);
            console.log(`Backed up ${configPath} to ${configBackupPath}`);
        }

        const linksPath = path.join(assetsDir, 'ems_links.json');
        const linksBackupPath = path.join(assetsDir, 'ems_links_backup.json');
        if (fs.existsSync(linksPath)) {
            fs.copyFileSync(linksPath, linksBackupPath);
            console.log(`Backed up ${linksPath} to ${linksBackupPath}`);
        }

        const rootConfigPath = path.join(baseDir, 'ems_config_utf8.json');
        const rootConfigBackupPath = path.join(baseDir, 'ems_config_utf8_backup.json');
